    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col gap-2">
                <h2 className="font-serif text-2xl font-light tracking-tight text-[var(--ink,#1a2420)]">
                    Your dashboard
                </h2>
            ...
            {!partner && (
                <div className="flex flex-col gap-3 rounded-2xl border border-[var(--champagne,#2f6b57)]/30 bg-white/70 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                ...
                <Card className="spotlight-frame border-[var(--champagne,#2f6b57)]/25 bg-[color-mix(in_srgb,var(--champagne)_8%,white)]">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium text-[var(--champagne,#2f6b57)]">Countdown</CardTitle>
                        <Clock className="h-4 w-4 text-[var(--champagne,#2f6b57)]" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold text-[var(--ink,#1a2420)]">
                                ...
                                return (
                                    <li key={step.label} className="relative rounded-2xl border border-border/50 p-4">
                                        <div className="mb-3 flex items-center gap-2">
                                            <span className={`rounded-full p-2 ${step.done ? 'bg-[var(--champagne,#2f6b57)]/15 text-[var(--champagne,#2f6b57)]' : 'bg-muted text-muted-foreground'}`}>
                    ...
                    {recentOrders.length > 0 ? (
                        recentOrders.map((order: any) => (
                            <Link key={order.id} href={`/account/orders/${order.id}`}>
                                <Card className="spotlight-frame border-border/60 bg-white/80 transition-colors hover:border-[var(--champagne,#2f6b57)]/30">
                        ...
                        <Card className="spotlight-frame border-dashed border-border/80 bg-white/60">
                            <CardContent className="space-y-4 py-10 text-center">
                                ...
                                <Button asChild className="rounded-full bg-[var(--champagne,#2f6b57)] text-black hover:bg-[var(--ink,#1a2420)] hover:text-white">
